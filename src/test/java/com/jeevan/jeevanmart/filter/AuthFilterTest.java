package com.jeevan.jeevanmart.filter;
...
class AuthFilterTest {
    ...
    void testUnauthenticatedRedirectWeb() throws IOException, ServletException {
        ...
        when(request.getRequestURI()).thenReturn("/jeevanmart/checkout");
        when(request.getContextPath()).thenReturn("/jeevanmart");

        authFilter.doFilter(request, response, chain);

        verify(response).sendRedirect("/jeevanmart/login?redirect=/jeevanmart/checkout");
    ...
    void testUnauthenticatedApiReturns401() throws IOException, ServletException {
        when(request.getSession(false)).thenReturn(null);
        when(request.getRequestURI()).thenReturn("/jeevanmart/api/v1/buyer/orders");
    ...
    void testBuyerCannotAccessAdmin() throws IOException, ServletException {
        ...
        when(request.getRequestURI()).thenReturn("/jeevanmart/admin/dashboard");
        when(request.getContextPath()).thenReturn("/jeevanmart");

        authFilter.doFilter(request, response, chain);

        verify(response).setStatus(HttpServletResponse.SC_FORBIDDEN);
        verify(response).sendRedirect("/jeevanmart/error/403");
        verify(chain, never()).doFilter(request, response);
    }
    ...
    void testBuyerCannotAccessSeller() throws IOException, ServletException {
        ...
        when(request.getSession(false)).thenReturn(session);
        when(session.getAttribute("currentUser")).thenReturn(buyer);
        when(request.getRequestURI()).thenReturn("/jeevanmart/seller/products");
        when(request.getContextPath()).thenReturn("/jeevanmart");
    ...
    void testSellerCannotAccessAdmin() throws IOException, ServletException {
        ...
        when(request.getRequestURI()).thenReturn("/jeevanmart/admin/users");
        when(request.getContextPath()).thenReturn("/jeevanmart");
    ...
    void testSellerCanAccessSeller() throws IOException, ServletException {
        ...
        when(request.getSession(false)).thenReturn(session);
        when(session.getAttribute("currentUser")).thenReturn(seller);
        when(request.getRequestURI()).thenReturn("/jeevanmart/seller/dashboard");
