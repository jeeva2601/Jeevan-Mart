...
class DtoTest {
    ...
    void testUserResponseSecurity() {
        ...
        User user = new User(1L, "Jeevan", "jeevan@test.com", "secret_hash", Role.SELLER, true, now);

        UserResponse userResponse = UserResponse.fromUser(user);
        assertNotNull(userResponse);
        assertEquals(1L, userResponse.getId());
        assertEquals("Jeevan", userResponse.getName());
        assertEquals("jeevan@test.com", userResponse.getEmail());
