...
```bash
git clone <your-repo-url> jeevanmart
cd jeevanmart
...
```bash
mvn compile
mvn exec:java -Dexec.mainClass="com.jeevan.jeevanmart.util.DbSeeder"
```
...
```bash
mvn clean package
cp target/jeevanmart.war $CATALINA_HOME/webapps/
$CATALINA_HOME/bin/startup.sh
```
Visit `http://localhost:8080/jeevanmart/`.
